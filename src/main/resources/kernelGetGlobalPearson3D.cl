...
kernel void kernelGetGlobalPearson3D(
    ...
    if(ref_var<threshold || ref_var==0.0f){
        pearson_map[ref_index] = 0.0f; // Set pixel to zero to avoid retaining spurious values already in memory
        return;
    }
            ...
            for(int x=bRW; x<image_width-bRW; x++){
                ...
                if(test_var<threshold){
                    ...
                    if(ref_std==0.0f && test_std==0.0f){
                        pearson_map[ref_index] += 1.0f * (float)weight; // Special case when both patches are flat (correlation would be NaN but we want 1 because textures are the same)
                    }else if(ref_std==0.0f || test_std==0.0f){
                        pearson_map[ref_index] += 0.0f; // Special case when only one patch is flat (correlation would be NaN but we want 0 because texture has no correlation with complete lack of texture)
                    }else{
                        pearson_map[ref_index] += (float)fmax(0.0f, (float)(covariance/((ref_std*test_std)+EPSILON))) * (float)weight; // Truncate anti-correlations
                    }
                }
            }
        }
